...
export default function Header() {
  return (
    <header className="hidden md:flex items-center border-b border-slate-200/50 dark:border-slate-700/50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md sticky top-0 z-50 shadow-sm">
      <nav className="max-w-7xl mx-auto px-6 lg:px-8 py-4 flex items-center justify-between w-full">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <span className="font-bold text-slate-950 dark:text-white text-base tracking-tight">
        ...
        <div className="flex items-center gap-1">
          {[['/', 'Home'], ['/about', 'About'], ['/careers', 'Careers']].map(([href, label]) => (
            <Link key={href} href={href}
              className="text-slate-700 dark:text-slate-300 hover:text-slate-950 dark:hover:text-white transition-colors font-medium text-sm relative px-3 py-2 rounded-lg hover:bg-slate-100/50 dark:hover:bg-slate-800/50 group">
              {label}
